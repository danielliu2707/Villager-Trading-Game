...
This file demonstrates the implementation of the Cave Class
...
class Cave:
    ...
    This is the class that represents the Cave Class Object, representing the functionality of a minecraft Cave
    
    Instance Attributes:
        name (str): A string representing the name of the Cave
        material (Material): A Material Class Object representing the material that can be obtrained from the Cave
        quantity (Float): The float value representing the quantity of the Material that can be obtained from the Cave
    """
    
    def __init__(self, name: str, material: Material, quantity: float=0) -> None:
        """
        
        This is the constructor magic method for the Cave Class, initializing a Cave Object
        ...
        # Assigning the appropriate paramater values to the relevant attributes
        self.name = name
    ...
    def add_quantity(self, amount: float) -> None:
        ...
        The is the method that increments the Cave's quantity by a specified amount

        Parameters:
            amount (Float): A float value representing the value by which Cave.quantity should be incremented by
        
        Pre-Conditions (What must be true for method to be called):
            amount must be a float/integer, cannot add a string or a boolean for eg. to an integer/float
        
        Post-Conditions (What is true after method is callled):
            the quantity instance attribute has been increased by amount

        Complexity:
            O(1)
        """
        # incrememnting the quantity of materials attributes
        self.quantity += amount 
    
    def remove_quantity(self, amount: float) -> None:
        """
        
        The is the method that decrements the Cave's quantity by a specified amount

        Parameters:
            amount (Float): A float value representing the value by which Cave.quantity should be decremented by

        Pre-Conditions (What must be true for method to be called):
            amount must be a float/integer, cannot add a string or a boolean for eg. to an integer/float
        
        Post-Conditions (What is true after method is callled):
            the quantity instance attribute has been decreased by amount
        
        Complexity:
            O(1)
        """
        # decrementing the quantity of materials attribute
        self.quantity -= amount

    def get_quantity(self) -> float:
        """
        
        This is the getter method that returns the quantity of the materials in the Cave

        Returns:
            self.quantity (float): A decimal that represents the amount of materials are in Cave

        Pre-Conditions (What must be true for method to be called):
            A cave object must be instantiated
        
        Post-Conditions (What is true after method is callled):
            In some way the value of the Cave's quantity has been used
        
        Complexity:
            O(1)
        """

        # Returning the quantity attribute
        return self.quantity

    def __str__(self) -> str:
        """
        
        The string magic method that returns the string representation of the Cave Object

        Returns:
            The String Representation of the Cave Class
        ...
        # Accessing the instance attributes relevant to the string representation
        return f"{self.name} {self.material} {self.quantity}"

    @classmethod
    def random_cave(self, material_list: list[Material]) -> Cave:
        """
        
        The Class Method that is returns a random Cave Class Object

        Parameters:
            material_list (list[Material]): A list of material objects from which a random material is chosen from to be mined from the cave
        
        Returns:
            A Cave Object that is randomly made

        Pre-Conditions (What must be true for method to be called):
            Must be a list of Materials Available
        
        Post-Conditions (What is true after method is callled):
            A random cave object will be produced
        
        Complexity:
            O(1)
        """
        
        # Getting the needed values to pass through to initialise a Cave Object
        number_materials = len(material_list)
        random_material_index = RandomGen.randint(0,number_materials-1)
        random_material_quantity = RandomGen.randint(1,10)
        # Returning a Cave Object
        return Cave(CAVE_NAMES[RandomGen.randint(0,len(CAVE_NAMES)-1)],material_list[random_material_index],random_material_quantity)
